...
const crawlAdventarRSS = async function (config, browser) {
    ...
    const adventarUrl = `https://adventar.org/calendars?year=2020`;
...
const crawlQiitaRSS = async function (config, browser) {
    ...
    const qiitaUrl = `https://qiita.com/advent-calendar/2020/calendars`;
    await page.goto(qiitaUrl);

    const paginationText = await (await page.$(`.st-Pager_count`)).innerText();
...
const main = async function () {
    ...
    // URL取得してリスト化
    const urls = [
        ...(await crawlAdventarRSS(config, browser)), 
        ...(await crawlQiitaRSS(config, browser)),
    ];
    console.debug(`${urls.length} URLs found`);

    await subscribeOnSlack(config, browser, urls);
